...
from utils import *
from DenseNet121_LP import *
from plots import *

# Setting logger
logging.basicConfig(
    filename="densenet.log",
    filemode="a",
    level=logging.INFO,
...
def lin_probe_densenet(device, train_loader, val_loader, classes, plot_path, lr=1e-3, num_epochs=30):    
    num_classes = len(classes)

    # Model
    model = DenseNet121_LP(num_classes)
...
def fine_tune_densenet(device, train_loader, val_loader, classes, strategy, lr, num_epochs, plot_path):
    num_classes = len(classes)
    model = DenseNet121_LP(num_classes)
    ...
    # Unfreeze last DenseNet block + classifier
    elif strategy == "last_block":
        for param in model.module.backbone.features.denseblock4.parameters():
            param.requires_grad = True
        trainable_params = list(model.module.backbone.features.denseblock4.parameters()) + list(model.module.classifier.parameters())
    ...
    # Allow only 20% of backbone parameters
        ...
        # so we unfreeze last two blocks (denseblock3 + denseblock4)
        for layer in [model.module.backbone.features.denseblock3, model.module.backbone.features.denseblock4]:
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for images, labels in train_loader:
            ...
            # epoch_norms = defaultdict(list)

            # for name, p in model.named_parameters():
            #     if p.grad is not None:
            #         epoch_norms[name].append(p.grad.norm().item())

            # grad_norms.append({k: np.mean(v) for k,v in epoch_norms.items()})
...
def layer_wise(data_dir, device, batch_size, seed, epochs, lr, plot_path):
    ...
    num_classes = len(classes)    
    model = DenseNet121_LP(num_classes=num_classes, enable_probe=True).to(device)
    model = torch.nn.DataParallel(model)
    layer_dims = {
        "block1": 256,
        "block2": 512,
        "block3": 1024,
        "block4": 1024   # DenseNet final block also outputs 1024 features
    }
