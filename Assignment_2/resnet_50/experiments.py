...
from utils import *
from ResNet50_LP import *
from plots import *

# Setting logger
logging.basicConfig(
    filename="resnet_50.log",
    filemode="a",
    level=logging.INFO,
...
Function name
    lin_probe_resnet50
Description
    This function performs linear probing on ResNet-50 backbone. All layers are 
    frozen and only the final linear classifier is trained on the target dataset.
    During validation, the function also extracts intermediate feature representations
    which are used for representation analysis (PCA, t-SNE)
Inputs
    device => A torch.device object. Either CUDA/CPU
    train_loader => A torch.utils.data.DataLoader that provides batches of training images and labels
    val_loader => A torch.utils.data.DataLoader that provides batches of val images and labels.
    classes => A list object of classes
Outputs
    Returns None
'''
def lin_probe_resnet50(device, train_loader, val_loader, classes, plot_path, lr=1e-3, num_epochs=30):    
    num_classes = len(classes)

    # Model
    model = ResNet50_LP(num_classes)
...
def fine_tune_resnet50(device, train_loader, val_loader, classes, strategy, lr, num_epochs, plot_path):
    num_classes = len(classes)
    model = ResNet50_LP(num_classes)
    ...
    # Unfreeze last ResNet block + classifier
        ...
        trainable_params = list(model.module.backbone.layer4.parameters()) + list(model.module.classifier.parameters())
    ...
    # Allow only 20% of backbone parameters
        ...
        # Reasoning: deeper layers adapt more to dataset-specific features,
        # so we unfreeze last two blocks (layer3 + layer4)
        for layer in [model.module.backbone.layer3, model.module.backbone.layer4]:
...
def run_data_efficiency_experiment(device, data_dir, strategy, subset_batch_size, lr, num_epochs, plot_path):
    # Define regimes
    ...
    for regime_name, fraction in regimes.items():
        ...
        for epoch in tqdm(range(num_epochs), desc=f"{strategy} - {regime_name}"):
            model.train()
    ...
    plot_val_accuracy_bar(results, strategy, plot_path)
    plot_train_val_gap(gaps, strategy, plot_path)
    return rel_drop
...
def layer_wise(data_dir, device, batch_size, seed, epochs, lr, plot_path):
    ...
    num_classes = len(classes)    
    model = ResNet50_LP(num_classes=num_classes, enable_probe=True).to(device)
