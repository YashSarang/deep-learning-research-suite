...
def main(args):
    ...
    model_resnet50 = ResNet50_LP(num_classes=0)
    ...
    macs, params = profile(model_resnet50, inputs=(dummy,), verbose=False)
    flops = 2 * macs
    logging.info(
    ...
    for strategy in strats:
        fine_tune_resnet50(device, train_loader, val_loader, classes, strategy, args.lr, args.epochs, args.plot_path)
    ...
    for strategy in strats:
        rel_drop = run_data_efficiency_experiment(device, args.data_dir, strategy, args.subset_batch_size, args.lr, args.small_epochs, args.plot_path)
        logging.info(
            f"Relative Performance Drop using {strategy}: {rel_drop:.4f}"
        )
