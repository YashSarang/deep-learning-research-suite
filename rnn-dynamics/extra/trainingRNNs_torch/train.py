...
def _targets_to_indices(y: torch.Tensor) -> torch.Tensor:
    """
    Accept:
      - one-hot: (N, C) float/bool  -> returns (N,) long
      - indices: (N,) int/long      -> returns (N,) long
      - indices: (N,1)              -> returns (N,) long
    """
    if y.ndim == 2 and y.shape[1] == 1:
        return y.squeeze(1).long()
    if y.ndim == 1:
        return y.long()
    # assume one-hot / distribution
...
def compute_loss_and_error(task, model, x, y_onehot, return_extras: bool = False):
    ...
    if classif == "lastSoftmax":
        # CE with one-hot or index targets
        y_idx = _targets_to_indices(y_onehot)
    ...
    if classif == "softmax":
        ...
        y_idx = _targets_to_indices(y_onehot)  # (T*B,) long
        loss = F.cross_entropy(logits_or_y, y_idx)
...
def main():
    ...
    for _ in range(n_eval_chunks):
        ...
        vy_t = to_torch(vy_np, device)
        vy = vy_t.float() if task_valid.classifType == "lastLinear" else vy_t
    ...
    dy_t = to_torch(dy_np, device)
    dy = dy_t.float() if task_diag.classifType == "lastLinear" else dy_t

    # One-time debug: verify label shapes & dtypes
    _dbx, _dby = task.generate(args.bs, int(args.max_length))
    print(f"[DEBUG] task={args.task} classifType={task.classifType} report={getattr(task,'report','?')}")
    print(f"[DEBUG] x_np: shape={_dbx.shape} dtype={_dbx.dtype}")
    print(f"[DEBUG] y_np: shape={_dby.shape} dtype={_dby.dtype}")
    print(f"[DEBUG] y_np sample (flat[:10]): {_dby.reshape(-1)[:10]}")
    if _dby.ndim == 2:
        print(f"[DEBUG] y_np row-sums[:5]: {_dby[:5].sum(axis=1)}")
    del _dbx, _dby
    ...
    while lr > 1e-8 and cont and n < args.maxiters - 1:
        ...
        y_t = to_torch(y_np, device)
        y = y_t.float() if task.classifType == "lastLinear" else y_t
        ...
        # Omega regularizer (Pascanu et al.) — skip entirely when alpha=0
        omega_val = 0.0
        steps_val = 0.0
        if use_omega:
